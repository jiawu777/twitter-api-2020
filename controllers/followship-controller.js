...
const followshipController = {
  addFollowing: (req, res, next) => {
    ...
    return Promise.all([
      ...
      if (!followingUser || followingUser.role !== 'user') return res.status(404).json({ status: 'error', message: '查無使用者' })
  ...
  removeFollowing: (req, res, next) => {
    ...
    return Promise.all([
      ...
      .then(([user, followship]) => {
        if (!user || user.role !== 'user') res.status(404).json({ status: 'error', message: '找不到這位使用者!' })
